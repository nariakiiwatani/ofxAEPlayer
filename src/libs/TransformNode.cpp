...
void TransformNode::refreshMatrix()
{
	...
	if(auto parent = std::static_pointer_cast<TransformNode>(getParent())) {
		if(isDirty(PARENT)) {
...
void TransformNode::pushMatrix() const
{
	ofPushMatrix();
	ofMultMatrix(*world_matrix_ptr_);
}

void TransformNode::popMatrix() const
{
	ofPopMatrix();
}
...
void TransformNode::addTranslationX(float x)
{
	translation_.x += x;
	dirty(LOCAL);
}

void TransformNode::addTranslationY(float y)
{
	translation_.y += y;
	dirty(LOCAL);
}
void TransformNode::addTranslationZ(float z)
{
	translation_.z += z;
	dirty(LOCAL);
}
